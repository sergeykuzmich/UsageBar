...
Under **⋯ → Show in Menu Bar**, pick whether the menu bar number tracks Claude Code, Codex, or whichever of the two is highest. With one provider pinned, **Show Both Windows** stacks its two readings — `5h 9%` over `7d 17%` — in two rows, which costs about 14pt of menu bar over the single number. Both choices are remembered across restarts.
