...
final class Updater {
    ...
    var currentVersion: SemanticVersion {
        SemanticVersion(Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "0")
            ?? SemanticVersion("0")!
    }
    ...
    private static func install(_ release: AppRelease, replacing bundle: URL) async throws {
        ...
        guard FileManager.default.fileExists(atPath: staged.appendingPathComponent("Contents/MacOS").path) else {
            throw AppUpdate.ParseError(message: "The downloaded archive did not contain \(bundle.lastPathComponent).")
        }
        guard FileManager.default.isWritableFile(atPath: bundle.deletingLastPathComponent().path) else {
            throw AppUpdate.ParseError(message: "\(bundle.deletingLastPathComponent().path) is not writable.")
        }
