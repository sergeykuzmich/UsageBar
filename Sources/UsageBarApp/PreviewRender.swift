  ...
  static func run(outputDirectory: String) async {
    ...
    empty.apply([
      ProviderStatus(
        kind: .claude,
        outcome: .unavailable(
          "`claude` was not found. Install it, or make sure it is on your login shell's PATH.")),
      ProviderStatus(
        kind: .codex,
        outcome: .unavailable(
          "`codex` was not found. Install it, or make sure it is on your login shell's PATH.")),
    ])
      ...
      ProviderStatus(
        ...
        outcome: .report(
          ProviderReport(
            ...
            windows: [
              UsageWindow(
                id: "a", windowMinutes: 300, usedPercent: 72,
                resetsAt: Date().addingTimeInterval(3600 * 2 + 600)),
              UsageWindow(
                id: "b", windowMinutes: 10080, usedPercent: 91,
                resetsAt: Date().addingTimeInterval(86400 * 3)),
              UsageWindow(
                id: "b2", windowMinutes: 10080, modelName: "Fable", usedPercent: 37,
                resetsAt: Date().addingTimeInterval(86400 * 3)),
            ]))),
      ProviderStatus(
        kind: .codex,
        outcome: .report(
          ProviderReport(
            ...
            windows: [
              UsageWindow(
                id: "c", windowMinutes: 300, usedPercent: 12,
                resetsAt: Date().addingTimeInterval(1800)),
              UsageWindow(
                id: "d", windowMinutes: 10080, usedPercent: 44,
    ...
    for status in live.statuses {
      print(
        "LIVE \(status.kind.rawValue): \(status.report.map { "\($0.windows.map(\.usedPercent))" } ?? status.unavailableReason ?? "?") stale=\(status.stale?.reason ?? "-")"
      )
    }
    fflush(stdout)
