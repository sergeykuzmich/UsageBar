...
public final class UsageStore {
  ...
  public func setProvider(_ kind: ProviderKind, enabled: Bool, now: Date = Date()) {
    if enabled {
      ...
      if inserted, let cached = cache[kind],
  ...
  public var menuBarReadout: MenuBarReadout {
    let scoped =
      menuBarSource.providerKind.map { kind in available.filter { $0.kind == kind } } ?? available
    let reports = scoped.compactMap(\.report)

    if showsBothWindows, menuBarSource.providerKind != nil, let report = reports.first {
      let windows = report.accountWindows.sorted {
    ...
    guard let highest = reports.compactMap(\.longestWindow).map(\.usedPercent).max() else {
      return .empty
    }
  ...
  private static func probe(_ kind: ProviderKind, skipping: Set<ProviderKind>) async
    -> ProviderStatus?
  {
  ...
  static func statuses(fromCache cache: [ProviderKind: CachedReport], now: Date) -> [ProviderStatus]
  {
    ProviderKind.allCases.compactMap { kind in
      guard let cached = cache[kind], now.timeIntervalSince(cached.fetchedAt) < staleLimit else {
        return nil
      }
      return ProviderStatus(
        kind: kind,
