...
public enum ExecutableLocator {
  ...
  public static func locate(_ name: String) -> URL? {
    ...
    let pathEntries = (ProcessInfo.processInfo.environment["PATH"] ?? "").split(separator: ":").map(
      String.init)
...
public enum Shell {
  ...
  public static func runLineProtocol(
    ...
    if !stderr.isEmpty {
      throw ShellError.failed(
        command: executable.lastPathComponent, exitCode: process.terminationStatus, stderr: stderr)
    }
    throw ShellError.timedOut(executable.lastPathComponent)
  }
  ...
  private static func childEnvironment() -> [String: String] {
    ...
    var seen = Set<String>()
    environment["PATH"] = (existing + defaults).filter { seen.insert($0).inserted }.joined(
      separator: ":")
