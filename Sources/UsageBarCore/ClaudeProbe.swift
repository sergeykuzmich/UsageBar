...
public enum ClaudeProbe {
  ...
  public static func probe(session: URLSession = .shared) async -> ProviderStatus {
    do {
      ...
      return ProviderStatus(
        kind: .claude,
        outcome: .report(ProviderReport(plan: auth.subscriptionType, windows: windows)))
    } catch let error as ShellError {
      return unavailable(error.errorDescription ?? "\(error)")
    } catch let error as ClaudeProbeError {
      return ProviderStatus(
        kind: .claude, outcome: .unavailable(error.message), retryAfter: error.retryAfter)
    } catch {
  ...
  private static func readAuth() throws -> ClaudeAuth {
    ...
    let result = try Shell.run(
      executable: executable, arguments: ["auth", "status", "--json"], timeout: 30)
  ...
  static func readAccessToken() throws -> String {
    ...
    if result.exitCode == 0,
      let token = try? ClaudeCredentialsParser.accessToken(from: Data(result.stdout.utf8))
    {
      return token
    }
    ...
    throw ClaudeProbeError(
      message:
        "Could not read Claude's credentials from the keychain. Approve the access prompt, or run `claude` to sign in again."
    )
  }
...
public enum ClaudeCredentialsParser {
  ...
  public static func accessToken(from data: Data) throws -> String {
    guard
      let token = (try? JSONDecoder().decode(Payload.self, from: data))?.claudeAiOauth?.accessToken,
      !token.isEmpty
    else {
      throw ClaudeProbeError(
        message: "Claude's stored credentials did not contain an access token.")
    }
    return token
  }
}
...
public enum ClaudeUsageParser {
  ...
  public static func windows(from data: Data) throws -> [UsageWindow] {
    ...
    let named = [("five_hour", 300, payload.fiveHour), ("seven_day", 10080, payload.sevenDay)]
      .compactMap { id, minutes, window -> UsageWindow? in
        ...
        return UsageWindow(
          ...
          resetsAt: window.resetsAt.flatMap(ISO8601.date(from:))
        )
      }
