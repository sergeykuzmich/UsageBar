...
struct ClaudeParserTests {
  ...
  @Test func parsesTimestampsWithoutFractionalSeconds() {
    #expect(
      ISO8601.date(from: "2026-08-03T06:20:00Z") == Date(timeIntervalSince1970: 1_785_738_000))
  }
  ...
  @Test func readsAnAccessTokenFromTheKeychainBlob() throws {
    let blob = Data(
      #"{"claudeAiOauth":{"accessToken":"sk-ant-oat01-xyz","expiresAt":1785758056844}}"#.utf8)
...
struct ClaudeRateLimitTests {
  static let usageBody = Data(
    #"{"five_hour":{"utilization":4.0,"resets_at":null},"seven_day":{"utilization":16.0,"resets_at":null}}"#
      .utf8
  )
  ...
  @Test func aShortRetryAfterWaitsAndThenSucceeds() async throws {
    ...
    let windows = try await ClaudeProbe.fetchWindows(
      token: "t", session: session, retryDelays: [0, 0])
...
struct PresentationTests {
  ...
  @Test func formatsTimeUntilReset() {
    ...
    #expect(
      RelativeTime.untilReset(now.addingTimeInterval(3600 * 3 + 1200), now: now)
        == "resets in 3h 20m")
    #expect(
      RelativeTime.untilReset(now.addingTimeInterval(86400 * 6 + 3600 * 4), now: now)
        == "resets in 6d 4h")
    #expect(
      RelativeTime.untilReset(now.addingTimeInterval(86400 * 25), now: now) == "resets in 25d")
  }
...
struct UsageStoreTests {
      ...
      outcome: .report(
        ProviderReport(
          ...
          windows: [
            ...
            UsageWindow(id: "seven_day", windowMinutes: 10080, usedPercent: 16, resetsAt: nil),
            UsageWindow(
              id: "weekly_scoped_fable", windowMinutes: 10080, modelName: "Fable", usedPercent: 82,
              resetsAt: nil),
          ])))
  ]
  ...
  @Test func aRetryAfterPenaltySurvivesARelaunch() {
    ...
    #expect(
      relaunched.fetchesToSkip(force: true, now: until.addingTimeInterval(-1)).contains(.claude))
    #expect(
      !relaunched.fetchesToSkip(force: true, now: until.addingTimeInterval(1)).contains(.claude))
  }
  ...
  @Test func aSuccessfulReadingClearsThePenalty() {
    ...
    #expect(!store.fetchesToSkip(force: true, now: until.addingTimeInterval(-1)).contains(.claude))
    #expect(
      !UsageStore(defaults: defaults).fetchesToSkip(force: true, now: until.addingTimeInterval(-1))
        .contains(.claude))
  }
