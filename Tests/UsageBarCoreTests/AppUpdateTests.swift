...
struct AppUpdateTests {
    ...
    @Test func rejectsAReleaseWithoutTheAppArchive() {
        let payload = Data(
            #"{"tag_name":"v9.0.0","assets":[{"name":"source.tar.gz","browser_download_url":"https://x/y"}]}"#.utf8
        )
    ...
    @Test func rejectsATagThatIsNotAVersion() {
        let payload = Data(
            #"{"tag_name":"nightly","assets":[{"name":"UsageBar.zip","browser_download_url":"https://x/y"}]}"#.utf8
        )
